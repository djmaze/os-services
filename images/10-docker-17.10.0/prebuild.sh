...
if [ "$ARCH" == "amd64" ]; then
  ARCH="x86_64"
  URL="https://download.docker.com/linux/static/edge/${ARCH}/docker-${VERSION}-ce.tgz"
fi
if [ "$ARCH" == "arm" ]; then
  DOCKERARCH="armhf"
  URL="https://download.docker.com/linux/static/edge/${DOCKERARCH}/docker-${VERSION}-ce.tgz"
  #DOCKERARCH="armel"
